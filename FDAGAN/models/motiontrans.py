...
from .streamconfiguration import StreamConfiguration, StreamConfigurationForSwapper

from FDAGAN.tools.human_digitalizer.bodynets import SMPL, SMPLH
from FDAGAN.tools.utils.filesio import cv_utils
from FDAGAN.tools.utils.geometry import cam_pose_utils
...
class Motiontrans(BaseRunnerModel):
    def __init__(self, opt, device=torch.device("cuda:0")):
        super(Motiontrans, self).__init__(opt)
        self._name = "Motiontrans"
        self.device = device
    ...
    def _create_networks(self):
        ...
        self.flow_comp = StreamConfiguration(opt=self._opt).to(self.device)
...
class Swapper(Motiontrans):
    ...
    def _create_networks(self):
        ...
        # 2. flow composition module
        self.flow_comp = StreamConfigurationForSwapper(opt=self._opt).to(self.device)
