...
class FDABlock(nn.Module):
    def __init__(self):
        super(FDABlock, self).__init__()

    def forward(self, q, k, v):
        """
        Args:
            q (torch.tensor): (N, FD, C, H, W)
            k (torch.tensor): (N, ns, FD, C, H, W)
            v (torch.tensor): (N, ns, FD, C, H, W)
        Returns:
    ...
    def query(self, q, k):
        ...
        """
        dk = k.shape[2]
        q = q.permute(0, 3, 4, 1, 2)        # (N, FD, C, H, W) - > (N, H, W, FD, C)
        k = k.permute(0, 4, 5, 2, 1, 3)     # (N, ns, FD, C, H, W) -> (N, H, W,  FD, ns, C)
        q.unsqueeze_(-1)
        alpha = torch.matmul(k, q) / math.sqrt(dk)      # (N, H, W, ns, 1)
        alpha = torch.softmax(alpha, dim=-2)            # (N, H, W, ns, 1)
        alpha = alpha.permute(0, 4, 3, 5, 1, 2)            # (N, ns, FD, 1, H, W)
...
class FDAB(nn.Module):
    ...
    def forward(self, tsf_x, src_x, Tst, temp_x=None, Ttt=None):
        ...
        src_warp = self.FDA(src_x, Tst.view(bs * ns, H, W, 2))         # (bs * ns, c2, h, w)
        src_warp_k = self.fk(src_warp)         # (bs * ns, c, h, w)
        src_warp_v = self.fv(src_warp)         # (bs * ns, c, h, w)

        K = [src_warp_k.view(bs, ns, FD, -1, h, w)] # (bs, ns, c, h, w)
        V = [src_warp_v.view(bs, ns, FD, -1, h, w)] # (bs, ns, c, h, w)

        if self.temporal and temp_x is not None and Ttt is not None:
            nt = Ttt.shape[1]
            # print(temp_x.shape, Ttt.shape)
            temp_warp = self.FDA(temp_x, Ttt.view(bs * nt, H, W, 2))   # (bs * nt, c, h, w)
            temp_warp_k = self.fk(temp_warp)   # (bs * nt, c, h, w)
            temp_warp_v = self.fv(temp_warp)   # (bs * nt, c, h, w)

            K.append(temp_warp_k.view(bs, nt, FD, -1, h, w))   # (bs, nt, c, h, w)
            V.append(temp_warp_v.view(bs, nt, FD, -1, h, w))   # (bs, nt, c, h, w)

        K = torch.cat(K, dim=1)     # (bs, ns + nt, c, h, w)
        V = torch.cat(V, dim=1)     # (bs, ns + nt, c, h, w)

        q = self.fq(tsf_x)  # (bs, c, h, w)
        Nq = q.view(bs, FD, -1, h, w)
        # attention query
        x = self.att_block(Nq, K, V)  # (bs, c, h, w)
...
def build_multi_stage_FDAB(src_num_filters, tsf_num_filters, temporal=True):
    assert len(src_num_filters) == len(tsf_num_filters)

    enc_FDABs = nn.ModuleList()
    # Down-Sampling
    ...
    for i in range(n_down):
        ...
        enc_FDABs.append(block)
    return enc_FDABs


def build_res_block_FDAB(src_res_channel, tsf_res_channel, n_res_block, temporal=True):
    res_FDABs = nn.ModuleList()
    # Down-Sampling
    for i in range(n_res_block):
        block = FDAB(
        ...
        res_FDABs.append(block)
    return res_FDABs


class BaseFDABGenerator(nn.Module, ABC):
    ...
    def forward_tsf(self, tsf_inputs, src_enc_outs, src_res_outs, Tst,
        ...
        for i in range(n_down):
            ...
            tsf_x = self.enc_FDABs[i](tsf_x, src_x, Tst, temp_x=temp_x, Ttt=Ttt)
        ...
        for i in range(len(self.res_blocks)):
            ...
            if temp_enc_outs is not None and Ttt is not None:
                ...
                temp_x = None
            tsf_x = self.res_FDABs[i](tsf_x, src_x, Tst, temp_x=temp_x, Ttt=Ttt)
...
class FDABGenerator(BaseFDABGenerator):

    def __init__(self, cfg, temporal=False):


        super(FDABGenerator, self).__init__()
        self._name = cfg.name
        ...
        self.enc_FDABs = build_multi_stage_FDAB(num_filters, num_filters, temporal)
        self.res_FDABs = build_res_block_FDAB(num_filters[-1], num_filters[-1], n_res_block, temporal)
        res_blocks = []
        for i in range(n_res_block):
            res_blocks.append(ResidualBlock(num_filters[-1], num_filters[-1]))
        self.res_blocks = nn.Sequential(*res_blocks)
...
class FDABFrontGenerator(BaseFDABGenerator):

    def __init__(self, cfg, temporal=False):


        super(FDABFrontGenerator, self).__init__()
