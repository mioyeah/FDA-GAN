...
from FDAGAN.tools.utils.morphology import CannyFilter
from FDAGAN.tools.utils.morphology import morph
from FDAGAN.tools.human_digitalizer.renders import SMPLRenderer
from FDAGAN.tools.utils.geometry import mesh



class StreamConfiguration(torch.nn.Module):
    ...
    def __init__(self, opt):
        super(StreamConfiguration, self).__init__()
        self._opt = opt
        self._name = "StreamConfiguration"
...
class StreamConfigurationForSwapper(StreamConfiguration):

    def __init__(self, opt):
        super(StreamConfigurationForSwapper, self).__init__(opt)
        self._opt = opt
        self._name = "StreamConfigurationForSwapper"

        self.all_faces_ids = list(range(self.render.nf))
        self.part_faces = list(self.render.body_parts.values())

    def make_uv_setup(self, bs, ns, nt, device):
        super(StreamConfigurationForSwapper, self).make_uv_setup(bs, ns, nt, device)
